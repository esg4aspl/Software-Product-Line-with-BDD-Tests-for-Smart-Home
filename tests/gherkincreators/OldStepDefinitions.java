...
public class OldStepDefinitions {
...
//				}
		...
		System.out.println(homeOutputs);
		
		if (!found) {
//			assertTrue(consoleExpectations.size() > 0);
			for (String e : consoleExpectations) {
				switch (e) {
					case "call fire department": assertTrue(getLast20().contains("FIRST_AID_GROUP calls Fire Department.")); homeOutputs.remove("FIRST_AID_GROUP calls Fire Department."); break;
					case "turn on siren": assertFalse(getLast20().contains("FIRST_AID_GROUP calls Fire Department.")); assertTrue(getLast20().contains("Siren responding to TURN_ON=Fire")); break;
				}
			}
		}
		
		this.consoleOutputIsAbout = "";
		consoleExpectations.clear();
	}
	...
	public void call_fire_department() { consoleExpectations.add("call fire department"); }
	...
	public void turn_on_siren() { 
		consoleExpectations.add("turn on siren");
	}
	...
	public void before(Scenario scenario) {
	    ...
	    consoleExpectations = new ArrayList<String>();
	...
	public void after(Scenario scenario) {
		if (!previousStep().equals("output to console") && !previousStep().equals("turn off"))
			output_to_console();
		app.home.kill();
		uiBag.clearMessages();
	...
	private String previousStep(int distance) {
		if (stepCounter > distance - 1)
			return steps[stepCounter - distance];
		return "";
	}
